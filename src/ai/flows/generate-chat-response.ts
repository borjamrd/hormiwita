...
const extractNameTool = ai.defineTool(
  ...
  async (input) => {
    ...
    // For this example, we'll assume a simple extraction or direct provision.
    // This tool is called by the LLM, so the LLM decides if the query contains a name.
    // The LLM will provide what it thinks is the name in the 'name' field of the output.
    // We are just defining the tool's capability. The LLM provides the actual extracted name.
    // This is a placeholder for the LLM to fill.
    return { name: undefined }; // The LLM will effectively "fill" this via its generation process.
  }
);
...
const extractObjectivesTool = ai.defineTool(
  ...
  async (input) => {
    // Similar to extractNameTool, the LLM uses its understanding to populate this.
    return { objectives: undefined }; // Placeholder for LLM to fill.
  }
);
...
const prompt = ai.definePrompt({
  ...
  output: { schema: GenerateChatResponseOutputSchema.shape.response }, // Prompt outputs only the chat response string
  prompt:
    // This part is effectively handled by the system prompt logic and tool interactions.
    // The LLM will use the system prompt, current query, userData, and chatHistory.
    // We construct the "user" turn for the LLM here.
    `{{#if chatHistory}}
...
const generateChatResponseFlow = ai.defineFlow(
  ...
  async (input) => {
    const llmResponse = await prompt(input); // LLM decides if/when to call tools
    ...
    for (const part of llmResponse.parts) {
      if (part.toolRequest) {
        // console.log('Tool request:', part.toolRequest);
        if (part.toolRequest.name === 'extractNameTool') {
          // Simulate tool execution based on LLM's belief it should run
          // The actual extraction quality depends on the LLM's ability to use the tool description
          // and the input it decides to pass to the tool (part.toolRequest.input).
          // For this example, we assume the LLM passes sensible input.
          // If the LLM *thought* a name was extractable and called the tool,
          // we trust its judgment for this simulation.
          // A more robust tool would perform actual extraction here.
          // We will assume the LLM provides the extracted name in the tool *output* if it found one.

          // The LLM is expected to generate a tool *response* if it calls a tool and gets output.
          // Let's assume the LLM itself is providing the 'extracted' data via its thought process
          // which leads to tool_code_execution output.
          // We need to see what the LLM *would* have outputted if the tool ran.
          // The `output` field in a ToolResponsePart is the data *from* the tool.
        } else if (part.toolRequest.name === 'extractObjectivesTool') {
          // Similar logic for objectives
        }
      }
      // Check for tool responses generated by the LLM after it 'calls' a tool
    ...
    // Only include updatedUserData if it has changed or has meaningful data
    ...
    return {
      ...
      updatedUserData: shouldIncludeUserData ? finalUserData : (input.userData || {}), // return existing if no change or new data
    };
  }
);
