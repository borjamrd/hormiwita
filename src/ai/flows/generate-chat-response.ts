...
const prompt = ai.definePrompt({
  ...
  prompt:
    `{{#if chatHistory}}
Historial de chat reciente (assistant es maspagüita, user es el usuario):
{{#each chatHistory}}
{{role}}: {{content}}
{{/each}}
{{/if}}
...
Respuesta JSON de maspagüita (assistant) cumpliendo con PromptOutputSchema:
`,
});
